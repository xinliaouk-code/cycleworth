...
export function RideAdviceCard() {
  const { lang } = useLanguage(); const [data, setData] = useState<Advice | null>(null)
  ...
  return <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm md:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium text-slate-400">{lang === 'zh' ? '今日骑行建议' : 'Today’s ride advice'}</p><h2 className={`mt-1 text-lg font-bold ${bad ? 'text-rose-600' : cautious ? 'text-amber-600' : 'text-emerald-600'}`}>{verdict}</h2><p className="mt-1 text-sm text-slate-600">{reason}</p></div><span className="rounded-xl bg-emerald-50 px-3 py-2 text-xl">🚲</span></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-5"><span className="rounded-lg bg-slate-50 p-2 text-slate-600">🌧️ {lang === 'zh' ? '降雨' : 'Rain'}<b className="mt-1 block text-slate-800">{data.rainProbability}%</b></span><span className="rounded-lg bg-slate-50 p-2 text-slate-600">💨 {lang === 'zh' ? '风' : 'Wind'}<b className="mt-1 block text-slate-800">{direction(data.windDirection)} {Math.round(data.windSpeed)} km/h</b></span><span className="rounded-lg bg-slate-50 p-2 text-slate-600">🌡️ {lang === 'zh' ? '温度' : 'Temp'}<b className="mt-1 block text-slate-800">{Math.round(data.temperature)}°C</b></span><span className="rounded-lg bg-slate-50 p-2 text-slate-600">🌇 {lang === 'zh' ? '日落' : 'Sunset'}<b className="mt-1 block text-slate-800">{sunset}</b></span><span className="rounded-lg bg-slate-50 p-2 text-slate-600">🍃 {lang === 'zh' ? '空气' : 'Air'}<b className="mt-1 block text-slate-800">{airText}</b></span></div></section>
}

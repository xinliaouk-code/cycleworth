...
export function MaintenanceCard({ userId, odometerKm }: { userId: string; odometerKm: number }) {
  ...
  return <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm md:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-medium text-slate-400">{text.title}</p>{!collapsed && <h2 className="mt-1 font-semibold text-slate-800">{maintenanceName(urgent, lang)}</h2>}</div><div className="flex items-center gap-3">{!collapsed && <span className={`text-sm font-bold ${color}`}>{maintenanceStatusText(progress.status, lang)}</span>}<button onClick={() => setCollapsed(value => !value)} title={collapsed ? text.expand : text.collapse} aria-label={collapsed ? text.expand : text.collapse} className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-base font-bold text-slate-600">{collapsed ? '+' : '−'}</button></div></div>{!collapsed && <><p className="mt-3 text-sm text-slate-500">{text.progress} {Math.round(progress.ratio * 100)}% · {progress.kmRemaining !== null && `${progress.kmRemaining.toFixed(0)} km ${text.remaining}`}{progress.kmRemaining !== null && progress.daysRemaining !== null && ' · '}{progress.daysRemaining !== null && `${progress.daysRemaining} ${text.days} ${text.remaining}`}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full" style={{ width: `${Math.min(100, progress.ratio * 100)}%`, background: `linear-gradient(90deg, #22c55e, ${maintenanceProgressColor(progress.ratio)})` }} /></div><div className="mt-4 flex justify-between"><button disabled={busy} onClick={complete} className="rounded-xl bg-slate-800 px-3 py-2 text-xs font-medium text-white disabled:opacity-50">{text.complete}</button><Link href="/maintenance" className="py-2 text-xs font-semibold text-sky-600">{text.all} →</Link></div></>}</section>
}

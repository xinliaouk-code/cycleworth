...
export function MaintenanceCard({ userId, odometerKm }: { userId: string; odometerKm: number }) {
  ...
  return <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm md:p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-medium text-slate-400">\u81ea\u884c\u8f66\u4fdd\u517b\u5efa\u8bae</p><h2 className="mt-1 font-semibold text-slate-800">{urgent.display_name}</h2></div><span className={`text-sm font-bold ${color}`}>{progress.status}</span></div><p className="mt-3 text-sm text-slate-500">\u5f53\u524d\u8fdb\u5ea6 {Math.round(progress.ratio * 100)}% · {progress.kmRemaining !== null && `${progress.kmRemaining.toFixed(0)} km \u5269\u4f59`}{progress.kmRemaining !== null && progress.daysRemaining !== null && ' · '}{progress.daysRemaining !== null && `${progress.daysRemaining} \u5929\u5269\u4f59`}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-sky-500" style={{ width: `${Math.min(100, progress.ratio * 100)}%` }} /></div><div className="mt-4 flex justify-between"><button disabled={busy} onClick={complete} className="rounded-xl bg-slate-800 px-3 py-2 text-xs font-medium text-white disabled:opacity-50">\u6807\u8bb0\u4e3a\u5df2\u5b8c\u6210</button><Link href="/maintenance" className="py-2 text-xs font-semibold text-sky-600">\u67e5\u770b\u5168\u90e8 →</Link></div></section>
}

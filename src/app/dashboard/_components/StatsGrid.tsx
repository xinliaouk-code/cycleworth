...
export function StatsGrid({ totalRides, totalDistanceKm, estimatedSavings, totalCalories }: StatsGridProps) {
  ...
  return <div className="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-4">{stats.map(([label, value, unit, labelClass, valueClass, noWrap]) => <div key={label} className="rounded-2xl border border-slate-100 bg-white p-3.5 shadow-sm md:p-6"><p className={`text-xs font-medium md:text-sm ${labelClass}`}>{label}</p><p className={`mt-1 text-xl font-bold md:mt-2 md:text-3xl ${valueClass} ${noWrap ? 'whitespace-nowrap text-[1.1rem] lg:text-2xl' : ''}`}>{value} {unit && <span className="text-xs font-normal text-slate-500 md:text-sm">{unit}</span>}</p></div>)}</div>
}

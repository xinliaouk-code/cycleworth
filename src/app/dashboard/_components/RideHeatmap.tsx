...
export function RideHeatmap({ rides }: { rides: Ride[] }) {
  const { lang, t } = useLanguage(); const ui = moduleText(lang); const [month, setMonth] = useState(() => new Date(new Date().getFullYear(), new Date().getMonth(), 1)); const [collapsed, setCollapsed] = useState(false)
  const weeks = useMemo(() => {
  ...
  return <section className={dashboardCardClass}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-2"><button onClick={() => setCollapsed(value => !value)} title={collapsed ? t.expand : t.collapse} className={`${dashboardCollapseButtonClass} mt-0.5`}>{collapsed ? '+' : '-'}</button><div><h2 className={dashboardCardTitleClass}>{ui.monthlyWeeklyRides}</h2>{!collapsed && <p className="mt-0.5 text-xs text-slate-400">{ui.weeklyDistance}</p>}</div></div>{!collapsed && <div className="flex shrink-0 items-center gap-2"><button aria-label="Previous month" onClick={() => setMonth(current => new Date(current.getFullYear(), current.getMonth() - 1, 1))} className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-slate-600">‹</button><span className="min-w-24 text-center text-sm font-semibold text-slate-700">{title}</span><button aria-label="Next month" onClick={() => setMonth(current => new Date(current.getFullYear(), current.getMonth() + 1, 1))} className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-slate-600">›</button></div>}</div>{!collapsed && <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">{weeks.map((week, index) => <div key={week.start.toISOString()} title={`${format(week.start)} – ${format(week.end)}: ${week.km.toFixed(1)} km`} className={`rounded-xl border p-3 ${color(week.km)}`}><p className="text-xs font-medium opacity-80">{lang === 'zh' ? `${ui.week}${index + 1} 周` : `${ui.week} ${index + 1}`}</p><p className="mt-1 text-[10px] opacity-75">{format(week.start)} – {format(week.end)}</p><p className="mt-2 text-lg font-bold">{week.km ? `${week.km.toFixed(1)} km` : '—'}</p></div>)}</div>}</section>
}

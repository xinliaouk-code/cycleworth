...
export function RideHeatmap({ rides }: { rides: Ride[] }) {
  const { lang } = useLanguage(); const [month, setMonth] = useState(() => new Date(new Date().getFullYear(), new Date().getMonth(), 1))
  const weeks = useMemo(() => {
